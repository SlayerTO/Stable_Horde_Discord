...
export default class extends Modal {
    ...
    override async run(ctx: ModalContext): Promise<any> {
        ...
        const pending_kudos = await ctx.database.query<{unique_id: string, target_id: string, from_id: string, amount: number}[]>("SELECT * FROM pending_kudos WHERE target_id=? LIMIT 1", [ctx.interaction.user.id]).catch(console.error)
        if(pending_kudos?.length) {

            const delete_entry = await ctx.database.query<{affectedRows: number}>("DELETE FROM pending_kudos WHERE target_id=? LIMIT 1", [ctx.interaction.user.id]).catch(console.error)
            if(!delete_entry?.affectedRows) return;
