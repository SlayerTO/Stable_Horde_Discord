...
ETA: <t:${Math.floor(Date.now()/1000)+(status?.wait_time ?? 0)}:R>`
        ...
        async function handlePartySubmit() {
            ...
            try {
                const update = await ctx.database?.query("UPDATE parties SET users=array_append(array_remove(users, ?), ?) WHERE channel_id=? RETURNING *", [ctx.interaction.user.id, ctx.interaction.user.id, ctx.interaction.channelId])
